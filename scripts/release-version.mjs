  ...
  try {
    ...
    if (validTags.includes(versionArg.toLowerCase())) {
      ...
      if (versionArg.toLowerCase() === 'autobuild') {
        ...
        // 格式: 2.3.0+daily.1004.a1b2c3d (使用最新 Tauri 提交)
        const latestTauriCommit = getLatestTauriCommit()
        newVersion = `${baseVersion}+daily.${generateShortTimestamp()}.${latestTauriCommit}`
      } else if (versionArg.toLowerCase() === 'deploytest') {
        // 格式: 2.3.0+patch.1004.cc39b27
        // 使用 Tauri 相关的最新 commit hash
        newVersion = `${baseVersion}+patch.${generateShortTimestamp(true, true)}`
      } else {
